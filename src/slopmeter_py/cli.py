...
from .providers import aggregate_usage, get_provider_availability, merge_provider_usage
...
def main(
    ...
    try:
        ...
        if fmt == "json":
            ...
            output_path.write_text(render_html_document(payload), encoding="utf-8")
        else:
            sections = build_render_sections(export_providers, aggregate_result.rows_by_provider)
            if fmt == "svg":
        ...
        print_run_summary(
            ...
            [provider.provider for provider in export_providers],
        )
